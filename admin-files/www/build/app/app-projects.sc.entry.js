...
class appProjects {
    ...
    async updateOrders() {
        ...
        await asyncForEach(this.pageData, async (item) => {
            await updateFromPath(`datasource/projects/${item._id}`, item);
        });
        getFromPath('datasource/projects')
            .then((resp) => {
            this.pageData = resp;
    ...
    renderList() {
        return this.pageData.map((item, index) => {
            const projectItem = item.languages[this.currentLanguage];
            return (h("li", { class: "card-list-item", key: `${Date.now()}-${index}`, "data-id": item._id },
                h("div", { class: "card-list-item-image" },
                    this.isAdmin ? (h("div", { class: "card-list-item-overlay" },
                        ...
                        h("button", { onClick: () => this.history.push(`/projects/${item._id}`), class: "btn btn-primary btn-sm", type: "button" }, "D\u00FCzenle"),
                        h("button", { class: "btn btn-light btn-sm holder", type: "button" },
                            h("span", { class: "fix-hamburger-md" })))) : '',
                    h("img", { width: 360, src: projectItem.image })),
            ...
            return (h("div", { class: "app-events" },
                h("header", { class: "page-header" },
                    h("div", { class: "page-header-body" },
                        ...
                        h("div", { class: "page-header-options" }, this.isAdmin ? (h("button", { onClick: () => this.history.push(`/add-project`), class: "btn btn-secondary" }, "Yeni Proje Ekle")) : ''))),
...
export { appProjects as AppProjects };
