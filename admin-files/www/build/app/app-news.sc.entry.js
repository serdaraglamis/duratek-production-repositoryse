...
import { d as deleteFromPath, b as getFromPath, c as updateFromPath } from './chunk-1931ffef.js';
...
class appNews {
    ...
    async updateOrders() {
        ...
        await asyncForEach(this.pageData, async (item) => {
            await updateFromPath(`datasource/news/${item._id}`, item);
        });
        getFromPath('datasource/news')
            .then((resp) => {
            this.pageData = resp;
    ...
    deleteItem(id) {
        swal({
            ...
            .then((willDelete) => {
            if (willDelete) {
                deleteFromPath(`datasource/news/${id}`)
                    .then(() => {
                    getFromPath('datasource/news')
                        .then((resp) => {
                        this.pageData = resp;
    ...
    renderList() {
        return this.pageData.map((item, index) => {
            const newsItem = item.languages[this.currentLanguage];
            return (h("li", { class: "card-list-item", key: `${Date.now()}-${index}`, "data-id": item._id },
                h("div", { class: "card-list-item-image" },
                    this.isAdmin ? (h("div", { class: "card-list-item-overlay" },
                        ...
                        h("button", { onClick: () => this.history.push(`/news/${item._id}`), class: "btn btn-primary btn-sm", type: "button" }, "D\u00FCzenle"),
                        h("button", { class: "btn btn-light btn-sm holder", type: "button" },
                            h("span", { class: "fix-hamburger-md" })))) : '',
                    h("img", { src: newsItem.image })),
            ...
            return (h("div", { class: "app-events" },
                h("header", { class: "page-header" },
                    h("div", { class: "page-header-body" },
                        ...
                        h("div", { class: "page-header-options" }, this.isAdmin ? (h("button", { onClick: () => this.history.push(`/add-news`), class: "btn btn-secondary" }, "Yeni Haber Ekle")) : ''))),
                h("main", { class: "page-body" },
                    h("ul", { class: "card-list" }, this.renderList())),
                h("footer", { class: "page-footer" })));
        }
    }
    static get is() { return "app-news"; }
