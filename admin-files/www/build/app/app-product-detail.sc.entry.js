...
class appProductDetail {
    ...
    async updateDetails() {
        if (this.productDetail.categories.length) {
            await updateFromPath(`products/${this.match.params.id}`, this.productDetail);
            const newProductId = this.match.params.id;
            async function asyncForEach(array, callback) {
                for (let index = 0; index < array.length; index++) {
                    await callback(array[index], index, array);
                }
            }
            let allcategories = cloneDeep(this.productCategories);
            let isChanged = true;
            if (lodash_isequal(this.oldCategories, this.productDetail.categories)) {
                isChanged = false;
            }
            else {
                siiimpleToast.message(`Ürünün eski kategorilerle olan ilişkisi siliniyor lütfen bekleyin...`);
                await asyncForEach(allcategories, async (item) => {
                    if (item.products.includes(newProductId) && this.oldCategories.includes(item._id) && !this.productDetail.categories.includes(item._id)) {
                        const indexof = item.products.indexOf(newProductId);
            ...
            if (isChanged) {
                await asyncForEach(this.productDetail.categories, async (item) => {
                    let cateItem = allcategories.find(a => a._id === item);
                    if (cateItem) {
                        if (!this.oldCategories.includes(cateItem._id)) {
    ...
    static get is() { return "app-product-detail"; }
...
export { appProductDetail as AppProductDetail };
